...
This chapter presents first some background knowledge required to understand \gls{Moca} design in \sect{moca-bg}.
Then we discuss the implementation details of \gls{Moca} in \sect{moca-design}.
After that present an experimental validation including an extensive comparison with existing memory trace collection tools in \sect{moca-expe}.
...
The monitoring thread is a kernel thread, that uses the \alg{monTh}, is in charge of enabling false page faults destined for \gls{Moca}.
