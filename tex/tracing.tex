...
\DB{Rewrite, requires defining complete, prcise}
...
\gls{Moca} consist of a Linux kernel module that can be loaded at runtime,  a script
...
%initialization. Thus the overhead of using \gls{Moca} and the Pin instrumentation
%is basically \gls{Moca} overhead plus one time the normal execution time.
...
%In short, \gls{Moca} collects complete traces at the granularity of the page, whereas instruction
%sampling cannot guarantee any completeness of the collected trace. And \gls{Moca} is 
...
%Furthermore \gls{Moca} traces provides information on time, thread sharing and CPU
%location of each captured accesses.

%Intercepting page faults instead of doing hardware sampling, guaranty that
%every page of the analyzed application will be intercepted. Therefore \gls{Moca}
...
%Furthermore while \gls{Moca} traces are \emph{complete} at the page granularity,
...
\begin{figure}[htb]
    ...
    \caption{Interactions betweens \gls{Moca} and Linux}
    \label{fig:moca}
\end{figure}
...
%Eventually, \gls{Moca} generates one csv file, each line of this file
...
%in charge of enabling false page faults destined for \gls{Moca}. It performs its task by
...
%The \texttt{MonitorThreadWakeUpInterval} \gls{Moca} parameter lets the user change the default setting
...
%which periodically reads \texttt{/proc} pseudo files used by \gls{Moca} kernel module to export
...
%\gls{Moca} has just to wake him up sufficiently often so that the kernel module does not run out
...
%end of our handler and the end of Linux page fault handler. To avoid that, \gls{Moca} stores
...
We evaluate \gls{Moca} by comparing it to the following state of the art tools. The first one,
...
%\GH{Pas vraiment}
%Although this is less precise than the information collected by \gls{Moca}, it does not require
...
%and take \gls{Moca} as reference. \GH{Incohérent avec le choix précédent de prendre tabarnac comme référence}
...
\fig{ovh} shows for each of the \gls{NPB}, the slowdown factor when
...
From \fig{ovh-Intel}, we can see that \gls{Mitos}, MitosTun overhead is
...
%Our experiments, show that \gls{Moca} has a good behavior for a wide range of
%parameters and helped us defining their default values. Our experiments also
%show that, with these parameters, \gls{Moca} provides significantly more precise traces
%than state of the art tools.
%Of course, because of this increased precision, \gls{Moca} is slower than two of these tools,
%\gls{MemProf} and \gls{Mitos}.
%Nevertheless, the comparison with \gls{Tabarnac} outlines the fact that, when a superset of the memory
%space is captured, \gls{Moca} efficiency is good.
...
%At the end of the day, \gls{Moca} is the only tool able to provide a
...
\DB{Cncl from PMBS, update add tabarnac}
...
To address this challenge, we propose \gls{Moca} an efficient tool for the collection of \emph{precise},
...
We evaluated \gls{Moca} by comparing it to two state of the art tools: \gls{Mitos} and
