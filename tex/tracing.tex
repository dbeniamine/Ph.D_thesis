...
This chapter presents first the design and usage of \gls{Tabarnac} through a use case and discuss the obtained results, its overhead and limits in \sect{Tabarnac}.
Then we present \gls{Moca} design and an extensive experimental validation in \sect{Moca}.
Finally we provide our conclusions on memory traces collection in \sect{Tools-Comp}.
...
\gls{Tabarnac} manage to keep it's overhead reasonable by two means.
...
This \gls{Pin} instrumentation has been replaced by a library that does not instrument the binary in the latest version of \gls{Moca}, as described in \sect{moca-design}.
