...
\begin{quote}
    \emph{Repeatability} concerns the exact repetition of an experiment, using the same experimental apparatus, and under the same conditions.

    \emph{Reproducibility} is the reproduction of the gist of an experiment: implementing the same general idea, in a similar setting, with newly created appropriate experimental apparatus.
\end{quote}
...
We consider an experiment as a three steps workflows depicted in \fig{exp-pipeline}, the first one calle \emph{experimental plan} defines all the experiment.
...
\begin{itemize}
    \item Experiment as a workflow, define:
        \begin{itemize}
            \item Experimental Plan
                \begin{itemize}
                    \item Machines /environment descriptions
                    \item Benchmarks
                    \item Metrics
                    \item References
                    \item Scripts to run the experiments
                    \item Scripts for both analysis
                    \item Full plan enable at least reproduce, at best repeat similar
                    \item Analysis are fully repeatable !
                \end{itemize}
            \item Raw analysis
                \begin{itemize}
                    \item Reduce input size (from 100Gio to few Mibs)
                    \item No statistics !
                    \item Parse log => from tool dependent output to generic csv
                    \item Raw analysis + Raw trace enables:
                        \begin{itemize}
                            \item Search for issues during the expe (meta data)
                            \item Search for issues during the parsing (data)
                            \item Compute new metrics / extract more data, repeat statistical analysis
                        \end{itemize}
                \end{itemize}
            \item Statistical Analysis:
                \begin{itemize}
                    \item Statistic + Visu
                    \item Represent data in a comprehensive way
                    \item Statistical scripts + data enables:
                        \begin{itemize}
                            \item Repeat statistic analysis, looking for maths errors
                            \item Try different visualization (visual bias)
                        \end{itemize}
                \end{itemize}
        \end{itemize}
\end{itemize}
...
\DB{Reorganize: explain application of the workflow described before\\
...
In this thesis, we ran two types of experiments: the fist one aims at analyzing the behavior of one application (mostly \gls{SOFA}) while the second one aims at comparing trace collection tools.
