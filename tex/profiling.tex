...
Then, we discuss the existing profiling tools that can be used to analyze the performance of application in \sect{prof-tools}.
...
Another approach to make performance counters more understandable consists in combining them with contextual informations.
...
To conclude, many tools were developed to monitor the performance of an application.
...
Several tools can help us making experiments more repeatable.
...
After that they duplicate their output to a file in this new directory and log every sensitive meta data as show in \lstr{log}.
...
\lstinputlisting[caption={Logging experimental informations.}, label=lst:log, float=htb,language=bash]{code/log-infos}
...
Each of the commands executed by these 3 steps are logged before execution as shown in \lstr{run}.

\lstinputlisting[caption={Execution of a run.}, label=lst:run, float=htb,language=bash]{code/do-run}
...
\begin{table}[t]
    ...
    \begin{tabular}{lccccc}
        ...
        \cmidrule(lr){2-6}
        & Linux 3.2.0-4 & \multicolumn{2}{c}{Debian Wheezy}  & \multicolumn{2}{c}{No hyperthreading}  \\
        \bottomrule
    \end{tabular}
