...
\begin{table}[t]
    ...
    \begin{tabular}{lrrrr}
        \toprule
        \multirow{2}{2cm}{CPU} & \multicolumn{1}{l}{Model} & Core & Threads & Frequency \\
        \cmidrule(lr){2-5}
        & \multicolumn{1}{l}{Intel Xeon E5-1607} & $4$ & $4$ & \SI{3.00}{Ghz} \\
        \midrule
        \multirow{2}{2cm}{Cache and Memory} & L1 size & L2 size & L3 size & Memory size \\
        \cmidrule(lr){2-5}
...
The hardware specifications of this machine are described in \tbl{hopi-hw}. It
runs a \gls{Debian} \emph{Wheezy} on a Linux kernel $3.2.0-4$ with hyper
threading disabled.

The experimental methodology described in \sect{expe-methodo} evolved during
...
\DB{Introduire resultats\\
    + Nom des scenes, eventuellement caractèristiques.\\
    + Parallelisation peu efficace voir très lente (choix des deux scènes).\\
    + Beaucoup d'idle Time => possibilité d'amélioration, voir côté mémoire ou
    I/O \\
    + Runtime ratio: on dépasse rarement les 30\% => hotspot non connus \\
    + Mem introduire courbe.
}

\begin{itemize}
    \item Expected behavior:
        \begin{itemize}
            \item Idle time ratio: low
            \item RUntime ratio: high, Most time should be spent in the observed functions
            \item Simulation time: Parallelisation should be at least as bad as sequential
...
\fig{SOFA-perfctr} shows a small example of the results that we were able to
observe with our performance evaluation. In this figure, we compare the
execution of a function called \texttt{applyDJT} that do ??? \DB{what does applyDJT does
already ?}, for two different \gls{SOFA} scenes. For the first scene
