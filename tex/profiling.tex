...
We consider an experiment as a three steps workflows depicted in \fig{exp-pipeline}, the first one called \emph{Experimental Plan} defines the whole experiment.
...
Preparing an experimental environment is not trivial: it should be \emph{minimalist} yet \emph{sufficient}.
Indeed if the environment is not sufficient and we have to install packages or library before the experiment, the installed version will depend on the time where the experiment is run making it almost impossible to repeat.
...
To do so, we can generate a filler set of (fake) results corresponding to our expectations.
From this set we can design some data visualizations.
Furthermore using this set we can add to the plot some text explaining how we expect the results to look.
Such information will forbid us from trying to explain a posteriori results that infirm our hypothesis.
...
We consider that a trace is \textbf{self explanatory} if and only if we can easily answer the following questions:
\begin{itemize}
    \item How the trace was generated, which means recovering the exact command that launched it, knowing the software used (including their versions and possible modifications) ?
...
\end{itemize}
To do so, all our experimental scripts starts the same way: they first create a new directory that will hold the traces and copy themselves with all the scripts which are on their directory inside it.
Then they duplicates their output to a file in this new directory and Log every sensible meta data.
During the experiment, every commands are echoed before executing them.
Concerning the data analysis we use \gls{R} as it provides a large set of reliable statistic analysis libraries.
Further more thanks to \gls{R-markdown}, we can produce a standalone structured output that contains the original questions, our assumptions, the results and plots, and our observations and comments.
...
To evaluate the variability and reduce the effect of external noise, we usually run each configuration at least $30$ times.
...
Yet, this is not enough to protect our experiment from system noise, indeed if at some point of the experiment a system process (such as logrotate) interfere with our experiment, the performance of a set of runs will drop.
As the runs are executed in order, it might be correlated with the size, and therefore we will not realize that this performance drop is due to external noise, and credit the size for it.
While if we randomize the runs, the performance drop will affect several runs (but not all) for different size.
As a result we will observe a high variability for the impacted sizes and therefore conclude that something might have interfered with the experiment.
Our experimental scripts generate a list of runs that should be executed, shuffle it and then execute them in a randomized order.
...
As it often dependent of external programs, it is preferable to use \gls{VCS} which manage dependencies and patches such as \gls{Git} or mercurial.
We distribute our experimental plans as \gls{Git} repositories, were each dependencies is listed as a submodule.
...
Distributing the raw analysis is more difficult as the trace file can be quite heavy and code repositories are usually not designed to handle large files.
...
\begin{itemize}
    ...
    \item \emph{Runtime ratio}: For each function, the ratio of the time spent in this function and the total time spent in the simulation loop.
        This metric is highlight functions that are actually hotspots.
    \item \texttt{L2}, \texttt{L3} and \texttt{MEM} predefined groups from \gls{Likwid}. These groups mainly compute the data volume exchanged at their level (respectively second level of cache, third level of cache and main memory) and the bandwidth.
        These two metrics and more precisely their evolution between sequential and parallel executions tell us how data is shared between the threads and if it shared efficiently or not.
...
Furthermore as \emph{Naskapi} is a personal machine and not a node from a cluster, we did not deployed an image of the system.
...
Moreover to correct an inefficient pattern we need to know precisely why it is inefficient, thus this tool should classify them in terms of performance issues.
