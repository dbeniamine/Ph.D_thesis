...
The easiest way to intercept a library call is by overriding it at runtime with the \texttt{LD\_PRELOAD} hack\DHm{ref?}\footnote{
...
Several tools can helps use making experiments more repeatable for instance, by running our experiment on a shared platform such as grid5000~\cite{Cappello05Grid5000}, we can hope that other people might access the same set of machines.
...
We consider an experiment as a three steps workflows depicted in \fig{exp-pipeline}, the first one called \emph{Experimental Plan} determines the whole experiment.
