...
The hardware details are summarized in \tbl{turing-hw}.
\texttt{Turing} runs version $3.13$ of the Linux kernel, while \texttt{Idfreeze} runs version $3.2$.

\DB{Distribute traces}
We evaluate the following applications with \gls{Tabarnac} in this paper: the \IS benchmark from the \gls{NPB} and \texttt{Ondes3D}.
...
\texttt{Ondes3D} is the main numerical kernel of the Ondes3D application~\cite{Dupros08Exploiting}.
...
\begin{figure}[htb]
    ...
    \caption{Access distribution and first-touch for structure
        \texttt{vz0} from \texttt{Ondes3D}.} %Original version on the left,
    %modified on the right.}
    \label{fig:ondes3d}
\end{figure}

The analysis of the accesses distribution in \texttt{Ondes3D} shows that each structure seems to be well distributed between the threads, as we can see for structure \texttt{vz0} in \fig{ondes3d-behaviour-vz0-orig}.
However, for all structures, thread~$0$ is responsible for all first accesses, as we can see for \texttt{vz0} in \fig{ondes3d-ft-vz0-orig}.
...
Such a modification results in the first touch distribution shown in \fig{ondes3d-ft-vz0-modif}, which is now distributed among all the threads.

\begin{figure}[htb]
    \centering
    \includegraphics[width=\linewidth]{tabarnac/ondes3d_exectime}
    \caption[Speedup for \texttt{Ondes3D}.]{Speedup for \texttt{Ondes3D} compared to the baseline.}
    \label{fig:ondes-res}
\end{figure}

We compare the performance of the modified version (\emph{First Touch}) to the original (\emph{Base}) version, running on the normal OS, with \emph{NUMA Balancing} activated and with an \emph{Interleave} policy that aims at reducing the impact of remote accesses.
...
\IS was executed with input class \emph{D} for the performance evaluation, resulting in a memory usage of $33.5$Gib, and class \emph{B} for the analysis, with a memory usage of $0.25Gib$.

The top of \fig{is-behaviour} shows the original access distributions for the three main structures of \IS.
We can see that each structure has a different access pattern: \texttt{key\_array}'s (\fig{is-behaviour-orig-kba}) access distribution shows that each thread works on a different part of the structure, which permits automated tools perform an efficient data/thread mapping on it.
On the other hand, \texttt{key\_buff2} (\fig{is-behaviour-orig-kb2}) is completely shared by all threads.
...
\lstinputlisting[caption=\IS code responsible for the Gaussian distribution of memory accesses to the \texttt{key\_buff1} structure., label=lst:is, float=!h]{code/is.c}

We can identify the source of this pattern in the \IS source code.
Indeed, all the accesses to \texttt{key\_buff1} are linear, except in one \gls{OpenMP} parallel loop where they depend on the value of except the ones shown in \lstr{is}\footnote{
...
As we noticed in \fig{is-behaviour-orig-kb1} that the values of \texttt{key\_buff2} follow a Gaussian distribution, we can design a distribution of the threads that provides both a good load balancing and locality of data.
...
With this code modification, we obtain the access distribution shown in the bottom of \fig{is-behaviour}.
...
As we can see in \fig{ovh}, on the Intel machine, the instrumentation slows the execution down by a factor from $10$ to $30$.
