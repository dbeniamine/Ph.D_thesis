...
On the other hand, \texttt{key\_buff2} (\fig{is-behaviour-orig-kb2}) is completely shared by all threads, we can barely see a diagonal pattern.
...
By default, \gls{OpenMP} threads are scheduled dynamically to avoid unbalanced distribution of work, but the developers also propose an optional cyclic distribution of the threads over the loop.
For our distribution, we split the loop into two equal parts and distribute each part among the threads in a cyclic way.
...
Indeed, we can now see a sharing pattern very similar to the one obtain for \texttt{key\_buff1}.
...
\begin{figure}[htb]
    \centering
    \includegraphics[width=.9\linewidth]{tabarnac/is_exectime}
...
Moreover, while simple static mapping policies can result in substantial improvements, the best policy depends on the memory accesses behavior of the parallel application.
Therefore, it is required to understand this behavior to select the most appropriate mapping policy.
