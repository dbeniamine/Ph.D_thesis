...
Although modern architectures embed a complex cache hierarchy, we will present the basic principles of these cache on an hypothetical machine with only one level of cache for the sake of simplicity.

\begin{itemize}
    \item Locality space / time
    \item 1 level cache
    \item Storage / eviction mechanisms
    \item Cache line
    \item Associativity
    \item Hierarchy and sharing
    \item Examples: Bad align / false sharing
\end{itemize}
...
\begin{itemize}
    \item Pagination
    \item Consequences for NUMA (first touch / mapping)
\end{itemize}
