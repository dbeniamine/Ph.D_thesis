...
\begin{enumerate}
    ...
    \item Retrieving the mapping between structures and pages, this step is the most costly one, but can be speed it up by several means:
        ...
        \end{itemize}
    \item Creating simplified data frames: At this point, the main data frames contains a set of access where shared access appears one time for each thread involved in the sharing as described in \fig{sharing-detection}. We can aggregate all theses accesses, reducing the size of the main data frame.
...
\begin{itemize}
    \item Results
        \begin{itemize}
            \item Small example
                \begin{itemize}
                    \item trace from fflas-ffpack, compiled with openblas
                        \DB{Refs fflas, openblas}
                    \item \fig{dgetrf-intensity} 
                        \begin{itemize}
                            \item POV: Nacc on page=fct(time)
                            \item 4 types of access disinguished
                            \item 6 data structures
                            \item 3 structs only private not that interesting => zoom
                            \item Stack weird pattern: small part used
                            \item Same mfor malloc \#1
                            \item Temporat pattern stack 1256
                        \end{itemize}
                    \item Easy one line \lstr{R-zoom}
                    \item \fig{dgetrf-share-zoom}
                        \begin{itemize}
                            \item Point of view Nsharing=f(time)
                            \item Similar pattern
                            \item Find something interesting to say
                        \end{itemize}
                \end{itemize}
        \end{itemize}
    \item  Discussion
        \begin{itemize}
            ...
            \item Labook: efficient for exploring, not for reproducing same analysis on different traces, or also need to master emacs
            \item Sofa => Limit: when lots of library
                \begin{itemize}
                    \item Retrieving mapping => slow
                    \item Impossible to visualize with facets
                    \item Need to do a priori filtering
                \end{itemize}
        \end{itemize}
\end{itemize}
...
Hypothetical tool \fig{generic-viewer}
\begin{itemize}
    \item Goal:
        \begin{itemize}
