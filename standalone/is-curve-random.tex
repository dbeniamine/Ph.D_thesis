...
\begin{axis}[
    ...
    \pgfplotsinvokeforeach {0,...,3}{
        \addplot[color={Col#1}] fill between[of=f0 and f10,
            soft clip={($(0,0)-(0,0)$) rectangle (.1,1)},];
        \addlegendentry{Thread #1}
    }
    % actual plots
    \foreach \fnum in {0,...,9}{
        \edef\doplot{\noexpand\addplot[draw=none,name path={f\fnum}, domain={0:8},forget plot] {0.\fnum*gauss(x)};}
        \doplot
    ...
    %ugly fix
    %\addplot[draw=none,name path=f11,domain={-.1:8.1},forget plot] {gauss(4)+0.001};
