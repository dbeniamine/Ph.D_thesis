...
\begin{tikzpicture}[font=\small]
    ...
    \begin{pgfonlayer}{bg}
        ...
        \path[pattern=north east lines, pattern color=ColB] (\balignsz+\eltsz*2,\vshift) rectangle (4*\eltsz,1+\vshift);
    \end{pgfonlayer}

    \draw[mybrace] (0,\vshift) -- (2*\eltsz,\vshift) node [midway,below=1em]
        {Fetch 0: \colg{\resid~useful lines} / \colb{\balign~useless lines}};
    \draw[mybrace] (2*\eltsz,\vshift) -- (4*\eltsz,\vshift) node [midway,below=1em]
        {Fetch 1: \colg{\balign~useful lines} / \colb{\resid~useless lines}};

    %% Good alignment
        \node[myarray={4}{none}{1}{good},txtnode] (good) at (0,-3) {\colg{\textbf{Good alignment:}}};

    \begin{pgfonlayer}{bg}
        \node[myarray={2}{ColG,mydash}{0}{invg},txtnode] (d1) at (0,-3) {};
    \end{pgfonlayer}

    \draw[mybrace] (0,\vshift-3) -- (2*\eltsz,\vshift-3) node [midway,below=1em]
