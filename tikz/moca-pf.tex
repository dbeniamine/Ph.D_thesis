...
\tikzstyle{ablock} = [block, fill=orange!30]
\tikzstyle{mblock} = [block, fill=blue!25, rounded corners]
\tikzstyle{lblock} = [block, fill=blue!10, rounded corners]
...
\begin{tikzpicture}[scale=0.8]
    ...
    \node [lblock]   (pf)  at (0,0) {Page fault Handler};
    % pages
    ...
    \uncover<8->{
        ...
        \node[mblock] (pfM) at ($(mon)+(0,-2)$) {Page fault Handler};
        \node[mblock] (trace) at ($(mon)+(4,0)$) {Trace};
        \node[mblock] (fpf) at ($(trace)+(0,-2)$) {False page faults};
