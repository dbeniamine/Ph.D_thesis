...
\begin{tikzpicture}[node distance = 2cm,scale=0.8]rotate=90
    ...
    \node [concept,right=2.5em of configs] (venv) {Virtual environment};

    \node [file, below=4em of configs] (filt) {Parsing script};
    \node [file, right of=filt] (main) {Main script};
    \node [file, right of=main] (ana) {Analysis script};

    \node [concept, below= 4em of main] (mach) {Experimental machine(s)};
    \end{pgfonlayer}

    \node [group,fit=(main) (filt) (ana),header={Experimental plan}
    {groupcol}{north}{}] (plan) {};

    \begin{pgfonlayer}{bg1}
        \node[distrib,fit=(configs) (venv) (plan),%
    ...
    \begin{pgfonlayer}{background}
        \path[dep] (configs.south) edge[out=270,in=90]
        node[above right=.5em and 1em,pos=.4] {depends} (main.north);
