...
\begin{tikzpicture}[node distance = 2cm,scale=0.8]rotate=90
    ...
    \begin{pgfonlayer}{foreground}
        ...
        \node[fileA, below right=.5em and 1.5em of csv] (ana2) {Analysis script};
    \end{pgfonlayer}

    \begin{pgfonlayer}{bg1}
        \node[distrib,fit=(raw) (mi) (filt1) (ana1) (csv),%
        header={Raw analysis}{distribcol}{north}{}] (rawa) {};
    \end{pgfonlayer}

    \node[fileR, above right=.5em and 2.5em of ana2] (res) {Analysis results};

    \begin{pgfonlayer}{bg1}
        \node[distrib,fit=(csv) (ana2) (res),%
        header={Statistic analysis}{distribcol}{north}{}] (stata) {};
    \end{pgfonlayer}
    ...
    \draw[exec] (raw.east) -- node[pos=.1,below=2em,rotate=90] {generate} (csv.west);
    \path[exec] (filt1.east) edge[out=0,in=180] (csv.west);
    \path[copy] (ana1.east) edge[out=0,in=180] (ana2.west);

    \draw[exec] (csv.east) -- node[pos=.7,below=2em,rotate=90] {generate} (res.west);
